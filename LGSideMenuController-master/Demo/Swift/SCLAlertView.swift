...
public class SCLAlertView: UIViewController {
    ...
    override public func viewWillLayoutSubviews() {
        ...
        let suggestedViewTextSize = viewText.sizeThatFits(CGSizeMake(viewTextWidth, CGFloat.greatestFiniteMagnitude))
    ...
    public func showTitle(title: String, subTitle: String, duration: TimeInterval?, completeText: String?, style: SCLAlertViewStyle, colorStyle: UInt?, colorTextButton: UInt?) -> SCLAlertViewResponder {
        ...
        for btn in buttons {
            btn.backgroundColor = viewColor
            btn.setTitleColor(UIColorFromRGB(colorTextButton!), for:UIControlState.Normal)
        }

        // Adding duration
        if duration > 0 {
            durationTimer?.invalidate()
            durationTimer = Timer.scheduledTimerWithTimeInterval(duration!, target: self, selector: Selector("hideView"), userInfo: nil, repeats: false)
        }
